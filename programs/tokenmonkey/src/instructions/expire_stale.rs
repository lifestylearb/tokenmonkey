...
use crate::events::ChallengeExpired as ChallengeExpiredEvent;
use crate::state::{CasinoConfig, Challenge, ChallengeStatus};

#[derive(Accounts)]
pub struct ExpireStale<'info> {
    /// Anyone can crank expiry (permissionless).
    ...
    /// Creator's USDC token account for refund.
    #[account(
        mut,
...
pub fn handler(ctx: Context<ExpireStale>) -> Result<()> {
    let clock = Clock::get()?;
    ...
    require!(
        ...
        TokenMonkeyError::ChallengeNotExpired
    );
    ...
    msg!("Challenge {} expired. Refunded {} USDC to creator", challenge.id, amount);
    Ok(())
}
